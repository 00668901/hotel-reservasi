...
async function initRooms() {
  try {
    ...
    console.error('Error initializing rooms - KV store table may not exist:', error)
    console.error('Please create the table kv_store_aa71f191 in your Supabase database')
  }
}
...
app.post('/make-server-aa71f191/signup', async (c) => {
  try {
    const { email, password, name } = await c.req.json()
    
    if (!email || !password || !name) {
      return c.json({ success: false, error: 'Email, password, dan nama harus diisi' }, 400)
    }
...
app.get('/make-server-aa71f191/stats', async (c) => {
  try {
    const reservations = await kv.get('reservations') || []
    const rooms = await kv.get('hotel_rooms') || []
    ...
    const upcomingReservations = reservations.filter((r: any) => {
      ...
      return r.status === 'confirmed' && checkIn >= today
    }).length
    ...
    const stats = {
      ...
      availableRooms: rooms.filter((r: any) => r.available).length
    }
